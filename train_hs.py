...
from models.networks import LatentHistogramShifting
from torch.utils.data import DataLoader, RandomSampler
from models.utils import PenaltyLoss, compute_psnr, overflow_num, quantize_residual_image, find_latest_model, \
    LaplacePrior, GaussianPrior
...
def train(args):
    ...
    model = LatentHistogramShifting(n_channels=args.c,
    ...
    with tqdm(total=(args.num_epoch - start_epoch + 1), position=0, desc="Epoch", ncols=100, leave=True) as epoch_bar:
        ...
        for epoch in range(args.num_epoch - start_epoch + 1):
            ...
            for cover, secret in tqdm(train_loader, position=1, desc=f"Epoch {epoch + 1} Iter", ncols=100, leave=False):
                ...
                stego, embed_secret, z, peaks, topk_counts, last_pos = model(cover, secret, hard_round=args.hard_round, reverse=False)
                ...
                epoch_bar.set_description(
                    desc=f"Epoch: {epoch + 1} / PSNR: {psnr:.2f}, Penalty: {args.lambda_penalty:.2f}, Pixels: {under_overflow_avg_num:.2f}, Total Loss: {total_loss:.4f}, Img Loss: {mse_loss:.4f}, Z Loss: {z_loss:.4f}, Penalty Loss: {penalty_loss:.4f}")
                global_epoch += 1
...
def train_print():
    ...
    parser.add_argument('--pixel_v', type=float, default=255)
    ...
    args.train_name = "LHS_" + generate_train_name_verbose(args)
    train(args)
